...
var componentFactor = function(config){
    ...
    config.height && $Div.css('hieght', config.height);
    ...
    $Div.on('cpLeave', function(){
        var self = this;
        setTimeout(function(){
        config.animateIn && $(self).animate(config.animateOut);            
        }, config.delay || 0);
    });
    $Div.on('cpLoad', function(){
        var self = this;
        setTimeout(function(){
            config.animateOut && $(self).animate(config.animateIn);
        }, config.delay || 0);
    });
    return $Div;
